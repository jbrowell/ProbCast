...
qreg_lightgbm <- function(data,
  ...
  # get list of features, and the label (response) column, to later make lightgbm dataset for each fold combination.
  ...
  output$models <- foreach::foreach(q = quantiles,.packages = c("lightgbm",pckgs),.options.snow = opts) %dopar% {
    ...
    for(fold in output$model_names){
      ...
      lgb_model = lightgbm::lgb.train(params=lgb_params, data=dataset, eval="quantile", alpha=j, num_threads=1) 
